    ...
    public class CustomerDAL : BaseDAL, ICommonDAL<Customer>
    {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"if exists(select * from Customers where Email = @Email)
                        select -1
                            ...
                            begin
                          insert into Customers(CustomerName, ContactName, Province, Address, Phone, Email, IsLocked,RoleNames)
                          values(@CustomerName, @ContactName, @Province, @Address, @Phone, @Email, @IsLocked,'customer');
        ...
        public List<Customer> List(int page = 1, int pageSize = 0, string searchValue = "")
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                var parameters = new
                {
                    page = page, //Bên trái là tên của tham số trong câu lệnh sql , Bên phải là giá trị truyền cho tham số 
                    pageSize = pageSize,
                    searchValue = searchValue
                };
